...
if st.button("Iniciar processamento", disabled=is_disabled):
    ...
    with st.status("Extraindo dados...", expanded=False) as status:
        ...
        if uploaded_reports_discharge:
            ...
            df_general, df_vigilance, df_smear, df_blood = fill_outcome(uploaded_reports_discharge, df_list)
        df_blood = filter_blood(df_blood)
        df_general, df_vigilance, df_smear, df_blood = compare_data(df_list, substitution_departments, {"df_general": materials_general, "df_vigilance": materials_vigilance, "df_smear": materials_smear_microscopy})
