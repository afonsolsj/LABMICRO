    ...
    if file_complete and file_removal:
        try:
            ...
            if col_comp and col_rem:
                ...
                st.download_button(label="Baixar (.xlsx)", data=output.getvalue(), file_name=nome_arquivo_novo, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            else:
                if not col_comp:
                    st.error(f"❌ A coluna de pedido não foi encontrada na 'Base Completa'. (Esperado: {possiveis_colunas})")
                if not col_rem:
                    st.error(f"❌ A coluna de pedido não foi encontrada nos 'Dados para Remoção'. (Esperado: {possiveis_colunas})")
        except Exception as e:
            st.error(f"⚠️ Ocorreu um erro inesperado: {e}")
    else:
        st.warning("⚠️ Por favor, anexe os dois arquivos para continuar.")